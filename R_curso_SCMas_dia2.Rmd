...
```{r}
DEM_COL<-raster("datos_espaciales/DEM_Colombia.tif")
DEM_COL
raster::plot(DEM_COL)
```
...
```{r}
dat_mamm<-read_csv("mamiferos_COL_GBIF.csv")
```
...
```{r}
DEM_prj<-raster("datos_espaciales/DEM_Colombia_prj.tif")
DEM_prj
```
...
departamentos<-st_read("datos_espaciales/COL_adm1.shp")
```
...
```{r}
#seleccionamos un polígono
HF<-raster("datos_espaciales/IHEH_2018.tif")
```
